...
- crates/kernel
  - 生成内核二进制文件
  - 大部分逻辑是通过组装其他模块形成最终产物
  - 内核的入口在 src/entry.S 中
  - 加载内核栈和临时页表后，跳转到 `__hart_entry()`
  - 主 hart 进行一些必要的初始化工作，并启动其他 hart
  - 最后进入 `kernel_loop()`，即内核主循环，不断运行用户任务
- crates/libkernel
  - 内核的主要逻辑所在
  - 包括 trap、process、memory、fs 等模块
- crates/arch
...
- [ ] extern_symbols 移出 libkernel
- [ ] 考虑 target-specific 依赖
- [ ] 添加文件系统的测试，包括且不限于：
  - 多次打开同一文件
  - append
  - lseek 超出文件末尾后再写
